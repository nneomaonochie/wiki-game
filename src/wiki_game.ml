  ...
  let rec traverse_links ~d ~curr_link path : string list option =
   (** let curr_link =
    ...
    then (
      ...
      let curr_link = match how_to_fetch with 
        | File_fetcher.How_to_fetch.Remote -> "https://en.wikipedia.org" ^ curr_link 
        | _ -> curr_link in 
      let link_descendents =
        get_linked_articles (* make sure you put the https//wikiipedia.org*)
          (File_fetcher.fetch_exn how_to_fetch ~resource:curr_link)
        |> List.filter ~f:(fun new_link ->
             d > 0 && not (Hash_set.mem has_visited new_link))
      in
      print_s [%message curr_link (link_descendents : string list) ]; 
      List.map link_descendents ~f:(fun new_link ->
        traverse_links ~d:(d - 1) ~curr_link:new_link (curr_link :: path))
